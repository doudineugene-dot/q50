...
```bash
Q50_OBU_CERT=obu-cert.pem ./build-epk.sh
```

Публичный сертификат OBU (X.509, PEM или DER) — всё, что нужно для сборки;
закрытый ключ DCU не требуется. Без сертификата скрипт соберёт `.epk` с нулевым
`m_key` — устройство его отвергнет, но так можно проверить приём формата.
